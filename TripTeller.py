...
def txt_to_epub(txt_file, epub_file,title):
    book = epub.EpubBook()

    # Set metadata
    book_title = title.encode('utf-8')
    book_id = hashlib.md5(book_title).hexdigest()[:6]
    book.set_identifier('id{}'.format(book_id))
    book.set_title("Exploring " + title)
    book.set_language('en')
    book.add_author('B.A.S.')

    # Create title page
    title_page = epub.EpubHtml(title='Title Page', file_name='title.xhtml', lang='en')
    title_page.content = f'<h1 style="text-align: center;">Exploring {title}</h1><br><h2>Your Guide to History, Culture, and Fun</h2>'
    title_page.content += '<style>body { display: flex; align-items: center; justify-content: center; height: 100%; }</style>'
    book.add_item(title_page)
    book.spine.insert(1, title_page)

    # Create ISBN and copyright page
    isbn_page = epub.EpubHtml(title='ISBN', file_name='isbn.xhtml', lang='en')
    isbn_page.content = f'<p style="text-align: center;">Copyright 2023</p>'
    isbn_page.content += '<style>body { text-align: bottom; }</style>'
    book.add_item(isbn_page)
    book.spine.insert(2, isbn_page)

    # Create dedication page
    dedication_page = epub.EpubHtml(title='Dedication', file_name='dedication.xhtml', lang='en')
    dedication_page.content = f'<h1 style="text-align: center;>Dedication</h1><br><p>' + PR("Please write me 1 paragraph for the dedication of a history, culture, reference, and travel guide for " + title + ", considering it was written by one person and should be a generic dedication to travelers and explorers.", "You are a professional writer for Lonely Planet writing the dedication page of a book about the history, culture, reference, and travel guide for small town of " + town + ".Make sure to vary your words and grammar so phrases do not repeat too much.", 0.5, 1024) + '</p>'
    dedication_page.content += '<style>body { display: flex; align-items: center; justify-content: center; height: 100%; }</style>'
    book.add_item(dedication_page)
    book.spine.append(dedication_page)

    # Read the input text file
    with open(txt_file, 'r') as f:
        txt_lines = f.readlines()

    # Initialize variables for the EPUB structure
    spine = ['nav']
    toc = []
    ...
    for line in txt_lines:
        if line.startswith('H1'):
            ...
            # Create a new Section for the chapter
            chapter_section = epub.Section(title)
            toc.append((chapter_section, []))
            current_chapter = chapter_section

        elif line.startswith('H2'):
            document.add_page_break()
            title = line[2:].strip()
            section = epub.EpubHtml(title=title, file_name=f'{title}.xhtml', lang='en')
            section.content = f'<h2>{title}</h2>'
            book.add_item(section)
            spine.append(section)

            # Add the section to the current chapter
            if current_chapter is not None:
                toc[-1][1].append(section)
                current_section = section

        elif len(spine) > 1:  # Ensure that there's a section to add content to
            content = line.strip()
            spine[-1].content += f'<p>{content}</p>'

    # Flatten the TOC hierarchy
    flat_toc = []
    for chapter, sections in toc:
        flat_toc.append(chapter)
        flat_toc.extend(sections)
    ...
    # Define the EPUB structure (spine)
    book.spine = spine

    # Save the generated EPUB file
...
def generate_amazon_description(savefile, town):
    prompt = "Write a compelling product description for an Amazon page selling the book 'Exploring " + town + ": Your Guide to History, Culture, and Fun.' The description should be 3-4 paragraphs long and should entice the reader to purchase the book. Make sure to mention some of the key features and highlights of the book, such as the comprehensive coverage of the town's history, culture, and attractions, as well as any unique aspects that set it apart from other travel guides. Use vivid language and persuasive writing techniques to create a sense of excitement and anticipation in the reader."
    response = PR(prompt, "You are a professional copywriter creating the product description for an Amazon page selling the book 'Exploring " + town + ": Your Guide to History, Culture, and Fun.' Your goal is to create a compelling and persuasive description that will convince readers to buy the book. Make sure to highlight the key features and highlights of the book, as well as any unique aspects that set it apart from other travel guides. Use vivid language and persuasive writing techniques to create a sense of excitement and anticipation in the reader.", 0.5, 1024)
    APD("\n\nAMAZON DESCRIPTION\n" + response, savefile + " AMZ.txt")
#------------------------------------------------------------------------------------------------------------------------------------------------
